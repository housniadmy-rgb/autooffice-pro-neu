...
import { dashboardMockup } from "../lib/dashboard-mockup"
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const urlLang = p.get("setLang")
    if (urlLang && translations[urlLang]) { localStorage.setItem("lang", urlLang); setLang(urlLang) }
    else { const s = localStorage.getItem("lang"); if (s && translations[s]) setLang(s); else setLang(detectLanguage()) }
    const handleClickOutside = (e: MouseEvent) => { if (dropdownRef.current && !dropdownRef.current.contains(e.target as Node)) setShowLangs(false) }
  ...
  const dash = dashboardMockup[lang] || dashboardMockup.en
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      {mobileMenu && <div className="sm:hidden mt-2 pb-2 border-t border-gray-100 pt-2"><a href="/" className="block text-sm text-gray-700 py-2" onClick={() => setMobileMenu(false)}>{t.home}</a><a href={l("login")} className="block text-sm text-gray-700 py-2" onClick={() => setMobileMenu(false)}>{t.login}</a><a href={l("registrieren")} className="block bg-[#1E40AF] text-white text-sm px-4 py-2 rounded-lg text-center mt-1" onClick={() => setMobileMenu(false)}>{t.register}</a><div className="mt-2"><button onClick={() => setShowLangs(!showLangs)} className="text-sm text-gray-500 py-1">{t.switchLang} ▼</button>
      ...
      <section className="py-3 sm:py-8 px-3 sm:px-6 max-w-4xl mx-auto text-center bg-[#F8FAFC]"><div className="border border-gray-200 rounded-2xl p-6 sm:p-10 bg-white"><h2 className="text-lg sm:text-2xl font-bold mb-2 sm:mb-4 text-gray-900">{demo.title}</h2><p className="text-sm sm:text-base text-gray-500 mb-4 sm:mb-6">{demo.desc}</p>
        <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-4 sm:p-6 max-w-2xl mx-auto mb-4 sm:mb-6 text-left"><div className="flex items-center justify-between mb-3 sm:mb-4"><p className="text-sm font-bold text-gray-900">{dash.title}</p><p className="text-xs text-gray-400">{dash.date}</p></div><div className="grid grid-cols-3 gap-2 sm:gap-3 mb-3 sm:mb-4"><div className="bg-blue-50 rounded-lg p-2 sm:p-3 text-center"><p className="text-base sm:text-lg font-bold text-[#1E40AF]">24</p><p className="text-xs text-gray-500">{dash.today}</p></div><div className="bg-green-50 rounded-lg p-2 sm:p-3 text-center"><p className="text-base sm:text-lg font-bold text-green-600">147</p><p className="text-xs text-gray-500">{dash.patients}</p></div><div className="bg-yellow-50 rounded-lg p-2 sm:p-3 text-center"><p className="text-base sm:text-lg font-bold text-yellow-600">98%</p><p className="text-xs text-gray-500">{dash.satisfied}</p></div></div><div className="border-t border-gray-100 pt-2 sm:pt-3 space-y-1"><div className="flex justify-between text-xs sm:text-sm"><span className="text-gray-700">{dash.apt1}</span><span className="text-green-600 font-medium">{dash.apt1s}</span></div><div className="flex justify-between text-xs sm:text-sm"><span className="text-gray-700">{dash.apt2}</span><span className="text-blue-600 font-medium">{dash.apt2s}</span></div><div className="flex justify-between text-xs sm:text-sm"><span className="text-gray-400">{dash.apt3}</span><span className="text-gray-400 italic">{dash.apt3s}</span></div></div></div>
